...
my_int =  read_tracert (sys.argv[3])

#my_int = get_ip(my_int)
doc_list = []
print (ips)
for ip in ips:
	my_int = []
	...
	for each in all_int:
		...
		if each.ip == ip:
			...
			try:	
				...
				in_error_rate = 0
			try:
				interface_info = each.interface_name + ' Mb/s in: '+ str(in_rate) +' Mb/s out: '+str(out_rate) + ' In Error rate: '+str(in_error_rate )+"%" +' Out Error rate: '+ str(out_error_rate )+"%"
